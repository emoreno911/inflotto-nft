...
contract InflottoTickets is ERC721URIStorage  {
    ...
    string private constant COLLECTION_NAME = "Inflotto Ticketss4";
    string private constant COLLECTION_ID = "IFLTK4";
    mapping(uint256 => Ticket) public tokenIdToTraits;

    string[] private palette = ["#EF5350", "#AB47BC", "#5C6BC0", "#42A5F5", "#26A69A", "#66BB6A", "#FFCA28", "#FFA726", "#FF7043", "#78909C"];
    ...
    function generateTicket(
        ...
        bytes memory path = abi.encodePacked(
            '<g transform="translate(0 871) scale(.1 -.1)" fill="',color,'">',
        ...
        bytes memory svg = abi.encodePacked(
            ...
            '<style>.base { font-family: Tahoma; font-weight: bold; }</style>', path,
            '<text font-size="5em" x="50%" y="50%" class="base" fill="#444" dominant-baseline="middle" text-anchor="middle">',index,'</text>',
            '<text font-size="3.5em" x="50%" y="60%" class="base" fill="#444" dominant-baseline="middle" text-anchor="middle">', "YTD",'</text>',
            '<text font-size="3em" x="50%" y="92%" class="base" fill="#FFF" dominant-baseline="middle" text-anchor="middle">', "DRAW ",draw,'</text>',
            '</svg>'
        );
        return string(
            abi.encodePacked(
    ...
    function getTraits(uint256 tokenId) private view returns (string memory, string memory, string memory) {
        return (
    ...
    function getTokenURI(uint256 tokenId) public returns (string memory){
        (string memory draw, string memory index, string memory color) = getTraits(tokenId);
        bytes memory traits = abi.encodePacked(
            '[',
            '{"trait_type":"Draw", "value":"',draw,'"},',
        ...
        bytes memory dataURI = abi.encodePacked(
            '{',
                '"name": "Inflotto Ticket #', tokenId.toString(), '",',
                '"description": "The Lottery of Inflation",',
                '"image": "', generateTicket(draw, index, color), '",',
                '"attributes": ', traits,
            '}'
        );
        return string(
    ...
    function mint(uint256 newItemId, uint256 draw, string memory index) public {
