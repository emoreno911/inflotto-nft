...
contract InflottoV2 is ChainlinkClient, VRFConsumerBase {
    ...
    function pickWinner() private {
        ...
        for (uint256 index = 0; index < tickets.length; index++) {
            int test = substractAndReturnAbs(int(tickets[index]), int(inflationToday));
    ...
    function getRandomNumber() public returns (bytes32 requestId) {
        require(LINK.balanceOf(address(this)) >= feeRandomness, "Not enough LINK - fill contract with faucet");
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        randomNumber = randomness % (INFLATION_UPPER - INFLATION_LOWER + 1) + INFLATION_LOWER;
